...
def main():
    ...
    # 初始化交易所
    binace_exchange = ccxt.binance({
        'timeout': 3000,
        'enableRateLimit': True,
    })

    # 加载行情
    markets = binace_exchange.load_markets()

    # 选择两个交易货币 A，B
    market_a = 'BTC'
    market_b = 'ETH'

    # 找到所有同时以 A 和 B 作为计价的货币
    # 市场内的交易对
    ...
    # 获取前一分钟的close价格  单位是毫秒
    last_min = binace_exchange.milliseconds() - 60 * 1000
    for base_coin in common_base_list:
        ...
        # 获取前一分钟的k线数据
        market_a2b_kline = binace_exchange.fetch_ohlcv(market_a2b_symbol, since=last_min, limit=1, timeframe='1m')
        market_b2c_kline = binace_exchange.fetch_ohlcv(market_b2c_symbol, since=last_min, limit=1, timeframe='1m')
        market_a2c_kline = binace_exchange.fetch_ohlcv(market_a2c_symbol, since=last_min, limit=1, timeframe='1m')
        ...
        # 防止超过rate limit
        time.sleep(binace_exchange.rateLimit / 1000)

    results_df.to_csv('./sanjiaotaoli.csv', index=None)
